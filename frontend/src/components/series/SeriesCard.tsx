...
export const SeriesCard: React.FC<SeriesCardProps> = ({ series, isSelected, onSelect }) => {
  ...
  return (
    <div
      ...
      className={`cursor-pointer rounded-xl p-3.5 transition-all duration-150 border text-left font-mono relative overflow-hidden group ${
        isSelected
          ? "bg-slate-900/95 border-cyan-500/90 shadow-lg shadow-cyan-950/60 ring-1 ring-cyan-500/40"
          : "bg-slate-900/50 hover:bg-slate-900/80 border-slate-800/90 hover:border-slate-700"
      }`}
    >
      {/* Selected Indicator Top Accent Bar */}
      {isSelected && (
        <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-cyan-400 via-emerald-400 to-cyan-500" />
      )}

      {/* Top Header: Series Target ID & Epoch Badges */}
      <div className="flex items-center justify-between gap-2 mb-2">
        ...
        <div className="flex items-center space-x-1.5 flex-shrink-0">
          {isDemoSeries && (
            <span className="px-1.5 py-0.5 text-[9px] font-bold rounded bg-cyan-950/80 text-cyan-300 border border-cyan-800/80">
              REFERENCE
            </span>
          )}
          <span
      ...
      <div className="text-xs text-slate-300 mb-2 flex items-center justify-between">
        <span className="text-slate-400 font-sans text-[11px]">Pass Interval:</span>
      ...
      <div className="flex flex-wrap items-center justify-between gap-1.5 text-[11px] pt-2 border-t border-slate-800/80">
        <div className="text-slate-400">
          Timespan: <span className="text-cyan-300 font-semibold">{series.timespanDays}d</span>
        </div>
      ...
      {series.boundsWgs84 && (
        <div className="mt-2 text-[10px] text-slate-400 truncate">
          AOI: [{series.boundsWgs84.minLon.toFixed(3)}°, {series.boundsWgs84.minLat.toFixed(3)}°] – [
